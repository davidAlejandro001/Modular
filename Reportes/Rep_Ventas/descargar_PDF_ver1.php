...
$tienda ='"San Juan Pablo II"';
$pdf->SetFont('helvetica','B',10); //Tipo de fuente y tamaño de letra
$pdf->SetXY(15, 20); //Margen en X y en Y
$pdf->SetTextColor(0,0,0);
$pdf->Write(0, 'Ferretienda: San Juan Pablo II');
$pdf->SetTextColor(0, 0, 0); //Color Negrita
$pdf->SetXY(15, 25);
//$pdf->Write(0, 'Ferretienda: '. $tienda);
...
$pdf->Cell(100,6,'Reporte de Ventas',0,0,'C');
...
$pdf->Cell(20,6,'No. Venta',1,0,'C',1);
...
if ($num_registro!=0) {
    ...
    while($dataRow=$stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $pdf->Cell(20,6,($dataRow['TVenta_id']),1,0,'C');
        $pdf->Cell(30,6,(date('d-m-Y', strtotime($dataRow['fecha']))),1,0,'C');
        $pdf->Cell(60,6,($dataRow['nombre'] . " " . $dataRow['apellidos']),1,0,'C');
        $pdf->Cell(25,6,($dataRow['total']),1,1,'C');
